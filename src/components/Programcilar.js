...
export default function Programcilar() {
  ...
  const [oneCikaninId, setOneCikaninId] = useState("");
  ...
  const oneCikaninIsmi = () => {
    ...
    return programcilar[oneCikaninId - 1].isim
  };
  const color = oneCikaninId == "" ? "royalblue" : "gold"
  const stil = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: color, // 🤔 kutlarken renk gold'a dönecek
  };
      ...
      <div className='programmers'>
        {
          ...
          programcilar.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.isim} <button onClick={() => {setOneCikaninId(dev.id)/* burada dev.id 'yi öne çıkan id'ye atayın */ }}>Kutla</button>
            </div>
          )
        }
      </div>
